...
const getArticleById = async (req, res) => {
    try {
        ...
        const article = await Article.findById(req.params.id).populate("author", "name email").lean();
        if (!article) {
...
const getArticleByGenre = async (req, res) => {
    try {
        ...
        const articles = await Article.find({ tags: tag }).populate("author", "name email").lean();
        res.status(200).json(articles);
    } catch (error) {
...
const getLatestArticles = async (req, res) => {
    try {
        const latestarticles = await Article.find().sort({ createdAt: -1 }).limit(4).populate("author", "name email").lean();
...
const getRandomArticles = async (req, res) => {
    try {
        const randomarticles = await Article.aggregate([{ $sample: { size: 4 } }]).populate("author", "name email").lean();
...
const getFeaturedArticles = async (req, res) => {
    try {
        const featuredArticles = await Article.find({ isFeatured: true }).populate("author", "name email").lean();
